...
class SingleSessionSingleTrialDataset(Dataset):
    def __init__(self, trial_data, hand_data, hand_timestamps, unit_spike_times, trial_id, bin_size=0.02, n_future_vel_bins=10):
        ...
        self.n_future_vel_bins = n_future_vel_bins
        ...
        self.spike_counts = torch.FloatTensor(self.spike_counts).transpose(0, 1)[
            :-self.n_future_vel_bins, :]  # shape (num_bins-n_future_vel_bins, n_neurons)
        ...
        for bin_idx in range(1, self.num_bins):
            self.velocity_matrix[:, bin_idx-1] = self.hand_matrix[:,
                                                                  bin_idx] - self.hand_matrix[:, bin_idx-1]
        self.velocity_matrix = torch.FloatTensor(self.velocity_matrix)

        # make the future velocity matrix with shape (2, num_bins-n_future_vel_bins-1, n_future_vel_bins)
        self.future_velocity_matrix = np.zeros(
            (2, self.num_bins-self.n_future_vel_bins-1, self.n_future_vel_bins))
        for bin_idx in range(self.num_bins-self.n_future_vel_bins-1):
            for future_idx in range(self.n_future_vel_bins):
                self.future_velocity_matrix[:, bin_idx,
                                            future_idx] = self.velocity_matrix[:, bin_idx + future_idx]

        self.future_velocity_matrix = torch.FloatTensor(self.future_velocity_matrix).permute(
            1, 0, 2)  # shape (num_bins-n_future_vel_bins-1, 2, n_future_vel_bins)

    def __len__(self):
        return 1

    def __getitem__(self, idx):
        return self.future_velocity_matrix, self.spike_counts[:-1], self.spike_counts[1:]
