...
class SingleSessionSingleTrialDataset_OtherWay(Dataset):
    def __init__(self, trial_data, hand_data, hand_timestamps, unit_spike_times, trial_id, bin_size=0.02, n_context_bins=50, vel_offset_bins=10):
        """
        ...
        self.vel_offset_bins = vel_offset_bins
        ...
        self.full_matrix = torch.cat(
            (self.spike_counts[:, :-1-self.vel_offset_bins], self.velocity_matrix[:, :-self.vel_offset_bins], self.velocity_matrix[:, self.vel_offset_bins:]), dim=0)

    def __len__(self):
        return (self.num_bins - self.n_context_bins - 1 - self.vel_offset_bins) // self.n_context_bins

    def __getitem__(self, idx):
        idx = idx * self.n_context_bins
        return self.full_matrix[:, idx:idx + self.n_context_bins]
